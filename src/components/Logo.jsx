...
export default function Logo({ size = "md", className = "" }) {
  ...
  return (
    <Link href="/" className={`flex items-center gap-2.5 group ${className}`}>
      <div
        className={`${s.box} relative flex items-center justify-center rounded-2xl bg-gradient-to-br from-[#FF6B2C] via-[#F2541B] to-[#E03A0E] shadow-lg shadow-orange-500/30 transition-transform group-hover:scale-105`}
      >
        <svg width={s.icon} height={s.icon} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M10.2 8.8c.2-.3.5-.4.8-.4.3 0 .6.1.8.4l.2.2.2-.2c.2-.3.5-.4.8-.4.3 0 .6.1.8.4.3.4.3 1 0 1.4l-1.4 1.5c-.2.2-.5.2-.7 0L10.2 10.2c-.3-.4-.3-1 0-1.4z" fill="#FF6B2C"/>
        </svg>
      </div>
      <span className={`${s.text} font-display font-extrabold tracking-tight text-foreground`}>
        Flavour<span className="text-[#F2541B]">Find</span>
      </span>
    </Link>
  );
}
